...
*Quneiform* is a command-line utility and graphical user interface that scans source and resource files to check for various i18n and l10n issues.
Additionally, the GUI version provides pseudo-translation support for *gettext* catalogs (\*.po files).
...
- Strings exposed for translation[^1] that probably should not be.
...
- Strings that contain extended ASCII characters that are not encoded.
  ...
  Encoding extended ASCII characters is recommended for best portability between compilers.<br />
  This check is turned off by default.
  When it is off, *Quneiform* instead checks for the opposite situation: strings with an escaped Unicode sequence that could simply be written as a literal UTF-8 character.
  For example, "Danke sch\\U000000F6n" instead of "Danke schön" will be reported as an issue.
...
- *Quarto* files containing:
  ...
  - Hyphens being used as range separators (en dashes should be used.)
...
- Increasing the width of the translations. \
  This is useful for ensuring that strings are not being truncated at runtime.
- Wrapping the translations in braces. \
  This is useful for ensuring that strings are not being pieced together at runtime.
- Appending a unique ID to each translation. \
  This is useful for finding where a translation is being loaded from.

When pseudo-translating, a copy of all string catalogs will be created and have their translations filled with mutations of their respective source strings.
