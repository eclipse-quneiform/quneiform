...
When this check is turned off, *Quneiform* instead checks for the opposite situation: strings that contain an escaped Unicode sequence (e.g., `\uXXXX`, `\UXXXXXXXX`, `\xHH`) that could simply be written as a literal UTF-8 character.
This is reported under the `escapedUnicode` warning ID.
...
This will also check for smart quotes, malformed shortcodes, and misused hyphens in *Quarto* files.

- `fontIssue`

Check for font issues.

This is performed on Windows *.rc files and checks for dialogs that use unusual font sizes or are not using \'MS Shell Dlg\'.[@ms-shell-dlg]
