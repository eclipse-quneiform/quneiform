...
- `escapedUnicode`: indicates that a string contains an escaped Unicode sequence (e.g., `\uXXXX`) that could simply be written as a literal UTF-8 character.
This is reported instead of `unencodedExtASCII` when that check is turned off (which is its default state).
