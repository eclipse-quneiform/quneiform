...
When that option is unchecked, *Quneiform* instead emits an `escapedUnicode` warning for strings that already contain an escaped Unicode sequence (e.g., `\uXXXX`) and recommends replacing it with the literal UTF-8 character.
:::
